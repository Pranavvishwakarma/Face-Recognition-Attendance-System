...
class Student:
    def __init__(self,root):
        ...
        #  #High level img to Low level img
        ...
        img_lf=Image.open("Images/attendance1.jpg")
        img_lf=img_lf.resize((730,130),Image.Resampling.LANCZOS)   #High level img to Low level img
        ...
        img_rt=Image.open("Images/attendance1.jpg")
        img_rt=img_rt.resize((700,125),Image.Resampling.LANCZOS)   #High level img to Low level img
        ...
        if self.var_dep.get()=="Select Department" or self.var_course.get()=="Select Course" or self.var_year.get()=="Select Year" or self.var_sem.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_std_email.get()=="" or self.var_gender.get()=="Select Gender" or self.var_dob.get()=="" or self.var_ForG.get()=="" or self.var_add.get()=="" or self.var_phone_no.get()=="" or self.var_cgpa.get()=="" or self.var_mentor.get()=="":
            ...
            try:
                ...
                if(update>0):
                    conn=mysql.connector.connect(host='localhost',user='root',password='abc',database='face_recognizer',auth_plugin='mysql_native_password')
                    my_cursor=conn.cursor()
                    sql="""UPDATE student SET dep=%s,course= %s,year= %s,semester= %s,name= %s,email= %s,gender= %s,phone= %s, address= %s,guardian=%s, dob= %s,cgpa= %s,teacher=%s,photosample= %s,eno=%s where Student_Id= %s"""
                    val=(
                        self.var_dep.get(),
                        self.var_course.get(),
                        self.var_year.get(),
